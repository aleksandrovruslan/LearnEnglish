package ru.aleksandrov.controllers;

import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import ru.aleksandrov.service.AddWordService;
import ru.aleksandrov.service.Handler;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.HashMap;

@WebServlet(name = "AddWordServlet", urlPatterns = "/addword")
public class AddWordServlet extends HttpServlet {
    private static final Logger log = LogManager.getLogger(AddWordServlet.class);
    private static final String TEMPLATE = "addword/addword";

    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        new Handler(new AddWordService(request, response, getServletContext()
                , new HashMap<>(), TEMPLATE)).performPost();
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        new Handler(new AddWordService(request, response, getServletContext()
                , new HashMap<>(), TEMPLATE)).performGet();
    }
}